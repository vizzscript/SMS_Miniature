...
public class UsernameValidationUtil {
    ...
    public static ResponseEntity<?> validate(String userName) {
        if (userName == null || userName.isEmpty()) {
            // throw new IllegalArgumentException("Username cannot be empty or null!!");
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body("Username cannot be empty or null!!");
        }

        if (userName.length() < MIN_LENGTH || userName.length() > MAX_LENGTH) {
            // throw new IllegalArgumentException(
            // "Username must be between " + MIN_LENGTH + " & " + MAX_LENGTH + "
            // characters.");
            return ResponseEntity.status(HttpStatus.PARTIAL_CONTENT)
                    .body("Username must be between " + MIN_LENGTH + " & " + MAX_LENGTH + " characters.");
        }

        if (!userName.matches(USERNAME_REGEX)) {
            // throw new IllegalArgumentException(
            // "Username must start with a letter and can contain letters, numbers, dots,
            // and underscores.");
            return ResponseEntity.status(HttpStatus.PARTIAL_CONTENT)
                    .body("Username must start with a letter and can contain letters, numbers, dots, and underscores.");
        }
        ...
        if (userName.contains("..") || userName.contains("_")) {
            ...
            // or underscores.");
            return ResponseEntity.status(HttpStatus.PARTIAL_CONTENT)
                    .body("Username cannot contain consecutive dots or underscores.");
        }
